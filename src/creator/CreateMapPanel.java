...
public class CreateMapPanel extends JPanel {
		...
		addMouseListener(new MouseListener() {
			...
			public void mousePressed(MouseEvent e) {
				if (e.getY() > WIDTH) {
					...
					char tmp = ' ';
					if (e.getX() > squareLength && e.getX() < squareLength * (mapDim - 1) && e.getY() > squareLength
							&& e.getY() < squareLength * (mapDim - 1)) {
						switch (selected) {
						case WALL:
							tmp = 'X';
							break;
	...
	protected void paintComponent(Graphics g) {
		...
		// for(int x = 0; x < 10; x++){
		// g.drawImage(wall, x*50, 0, (x+1)*50, 50, 0, 0, wall.getWidth(),
		// wall.getHeight(), null);
		// g.drawImage(wall, x*50, 450, (x+1)*50, 500, 0, 0, wall.getWidth(),
		// wall.getHeight(), null);
		// }
		//
		// for(int y = 1; y < 9; y++){
		// g.drawImage(wall, 0, y*50, 50, (y+1)*50, 0, 0, wall.getWidth(),
		// wall.getHeight(), null);
		// g.drawImage(wall, 450, y*50, 500, (y+1)*50, 0, 0, wall.getWidth(),
		// wall.getHeight(), null);
		// }
		...
		for (int x = 0; x < mapDim; x++) {
			for (int y = 0; y < mapDim; y++) {
				switch (map[y][x]) {
				case 'X':
					g.drawImage(wall, (int) (x * squareLength), (int) (y * squareLength),
							(int) ((x + 1) * squareLength), (int) ((y + 1) * squareLength), 0, 0, wall.getWidth(),
							wall.getHeight(), null);
					break;
				case 'R':
					g.drawImage(rock, (int) (x * squareLength), (int) (y * squareLength),
							(int) ((x + 1) * squareLength), (int) ((y + 1) * squareLength), 0, 0, rock.getWidth(),
							rock.getHeight(), null);
					break;
				case 'E':
					g.drawImage(exit, (int) (x * squareLength), (int) (y * squareLength),
							(int) ((x + 1) * squareLength), (int) ((y + 1) * squareLength), 0, 0, rock.getWidth(),
							rock.getHeight(), null);
					break;
				default:
	...
	public int checkMapValidity() {
		...
		if (exitCounter == 0) {
			JOptionPane.showMessageDialog(getParent(), "Place at least one exit.", "Error", JOptionPane.ERROR_MESSAGE);
			return 7;
		} else if (exitCounter > 1) {
			JOptionPane.showMessageDialog(getParent(), "Place only one exit.", "Error", JOptionPane.ERROR_MESSAGE);
			return 8;
		}
		...
		for (int x = 0; x < mapDim; x++) {
			for (int y = 0; y < mapDim; y++) {
				if (map[y][x] == 'E') {
					if (((x == 0 || x == mapDim - 1) && y == x) || (x == 0 && y == mapDim - 1)
							|| (y == 0 && x == mapDim - 1)) {
						JOptionPane.showMessageDialog(getParent(), "Exit cannot be placed in a corner.", "Error",
